...
class Settings(BaseSettings):
    ...
    nutritionist_fallback_id: int = Field(default=1, env="NUTRITIONIST_FALLBACK_ID")

    # AWS — boto3 uses the default credential chain (IAM role in prod, ~/.aws or env locally)
    aws_access_key_id: Optional[str] = Field(default=None, env="")
    ...
    revenuecat_api_key_ios: Optional[str] = Field(default=None, env="REVENUECAT_API_KEY_IOS")
    ...
    # Testimonial clients: always shown the daily-pass coupon box (comma-separated client IDs)
    testimonials_client_ids: str = Field(default="", env="TESTIMONIALS_CLIENT_IDS")
