...
class UvicornNoiseFilter(logging.Filter):
    """Drops uvicorn's per-WebSocket protocol chatter while keeping
    actual errors and startup/shutdown events.

    uvicorn emits these at INFO on `uvicorn.error` (yes, the logger is
    misleadingly named — it carries lifecycle too, not just errors):
      - `('1.2.3.4', 0) - "WebSocket /api/..." [accepted]`
      - `('1.2.3.4', 0) - "WebSocket /api/..." 403`
      - `connection open` / `connection closed` / `connection rejected`
    These add no signal beyond our own `gymmate.chat.ws` logs.
    Errors (stack traces, ASGI exceptions) are at ERROR level — they
    pass straight through.
    """

    _DROP_PREFIXES = ("connection open", "connection closed", "connection rejected")

    def filter(self, record: logging.LogRecord) -> bool:
        if record.levelno >= logging.WARNING:
            return True
        msg = record.getMessage()
        if msg.startswith(self._DROP_PREFIXES):
            return False
        # Per-connection WebSocket handshake lines from uvicorn.protocols
        if 'WebSocket' in msg and ('[accepted]' in msg or '" 403' in msg):
            return False
        return True
...
def build_logging_config() -> dict:
    ...
    return {
        ...
        "filters": {
            ...
            "uvicorn_noise": {"()": UvicornNoiseFilter},
        },
        "formatters": {
            "json": {
                "()": JsonFormatter,
        ...
        "handlers": {
            ...
            # Same as stdout, but also drops uvicorn WS protocol chatter.
            "stdout_uvicorn": {
                "class": "logging.StreamHandler",
                "filters": ["request_context", "uvicorn_noise"],
                "formatter": "json",
            },
            # Swallow noisy framework loggers like uvicorn.access.
            "null": {
        ...
        "loggers": {
            ...
            # uvicorn.error stays at INFO so startup/shutdown are visible
            # and real errors (ERROR/CRITICAL) come through. The filter
            # silently drops only the per-connection lifecycle noise.
            "uvicorn.error": {
                "handlers": ["stdout_uvicorn"], "level": level, "propagate": False,
            },
