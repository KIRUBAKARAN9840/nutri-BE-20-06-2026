...
from app.fittbot_admin_api.export_logs import export_logs
from app.fittbot_admin_api.nutrition_activity import nutrition_activity
...
from app.fittbot_admin_api.nutritionist_sessions.create_session import router as nutritionist_create_session_router
...
@router.get("/api/temp/create_membership_session_table")
async def create_membership_session_table():
    from app.models.database import engine
    from app.models.nutrition_models import NutritionMembershipSession
    try:
        NutritionMembershipSession.__table__.create(engine, checkfirst=True)
        return {"status": "success", "message": "nutrition_membership_sessions table created or already exists"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
router.include_router(export_logs.router)
router.include_router(nutrition_activity.router)
...
router.include_router(nutritionist_create_session_router)
