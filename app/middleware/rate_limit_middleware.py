...
class AbusePrevention:
    ...
    async def is_jailed(self, subject: str) -> Tuple[bool, int]:
        """Check if subject is currently jailed. Returns (jailed, retry_after_secs)."""
        try:
            jail_key = f"jail:{subject}"
            ttl = await self.redis.ttl(jail_key)
            if ttl > 0:
                return True, ttl
        except Exception:
            pass
    ...
    async def record_strike(self, subject: str) -> None:
        """Record a rate-limit violation strike and jail if threshold is reached."""
        try:
            strike_key = f"jail:strikes:{subject}"
            strikes = await self.redis.incr(strike_key)
            # Strikes expire after 24 hours of good behavior
            await self.redis.expire(strike_key, 86400)

            # Check if we need to jail
            ban_duration = 0
            for threshold, duration in JAIL_ESCALATION:
                if strikes >= threshold:
                    ban_duration = duration

            if ban_duration > 0:
                jail_key = f"jail:{subject}"
                await self.redis.setex(jail_key, ban_duration, "1")
                logger.warning(
                    "Subject jailed: subject=%s strikes=%d ban_seconds=%d",
                    subject, strikes, ban_duration,
                )
        except Exception as e:
            logger.warning("Failed to record strike for %s: %s", subject, e)
