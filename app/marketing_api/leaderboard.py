...
from app.services.date_utils import week_range, month_range
...
def get_date_filter(period: str, start_date: Optional[str], end_date: Optional[str]):
    ...
    if period == "weekly":
        ws, we = week_range(today.date())
        start = datetime.combine(ws, datetime.min.time())
        end = datetime.combine(we, datetime.min.time())
    elif period == "monthly":
        ms, me = month_range(today.date())
        start = datetime.combine(ms, datetime.min.time())
        end = datetime.combine(me, datetime.min.time())
