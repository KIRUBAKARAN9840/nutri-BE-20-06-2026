...
from app.services.date_utils import start_of_day as sod, end_of_day as eod
import pytz
...
async def get_team_current_attendance(manager_id: str, user_id: int, role: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        start_of_day = sod(today)
        end_of_day = eod(today)
...
async def get_bde_attendance(
    ...
    try:
        ...
        start_of_day = sod(selected_date)
        end_of_day = eod(selected_date)
