...
class ExportInfo(Base):
    """Audit log for every successful data export performed by admin/employee users."""
    __tablename__ = "export_info"
    __table_args__ = {"schema": "fittbot_admins"}

    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    name = Column(String(150), nullable=False, index=True)         # Full name of the exporter
    role = Column(String(100), nullable=False, index=True)         # Role of the exporter (admin / employee role)
    export_page_url = Column(String(500), nullable=False)          # Frontend page URL from which export was triggered
    created_at = Column(DateTime, default=datetime.now, nullable=False)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now, nullable=False)


class UserLoginLogs(Base):
    """Activity tracking log for successful user login/logout and session durations."""
