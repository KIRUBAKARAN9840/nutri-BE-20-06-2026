...
class PromoCampaignRedemption(DailyPassBase):
    ...
    __table_args__ = (
        # Per-user cap is enforced in application code (count < 3); a plain
        # index keeps the (campaign_id, client_id) count query fast.
        Index("ix_pcr_campaign_client", "campaign_id", "client_id"),
        {"schema": "dailypass"},
    )
