...
from app.utils.ai_sanitizer import SAFE_IMAGE_MAX_PIXELS, sanitize_food_name, validate_nutrition_output
...
Image.MAX_IMAGE_PIXELS = SAFE_IMAGE_MAX_PIXELS
...
def analyze_food_text(
    ...
    try:
        ...
        # Security: sanitize food item names before validation
...
# ─── COMPRESSION-ONLY CELERY TASK (CPU-bound only) ──────────────────────────
@celery_app.task(bind=True, name="app.tasks.image_scanner_tasks.compress_food_images")
def compress_food_images(
    self,
    raw_image_data_list: List[dict],
) -> List[dict]:
    """
    CPU-bound only: validate and compress images.
    AI analysis is done inline in FastAPI (I/O-bound).

    Args:
        raw_image_data_list: List of dicts with raw_data (base64), content_type, filename

    Returns:
        List of {"compressed_b64": str, "content_type": str}
    """
    task_id = self.request.id
    compressed_images = []
    ...
    try:
        for idx, image_data in enumerate(raw_image_data_list):
            ...
            compressed, ct = _validate_and_compress_raw_image(raw_bytes, content_type, filename)
            compressed_b64 = base64.b64encode(compressed).decode('utf-8')
            compressed_images.append({"compressed_b64": compressed_b64, "content_type": ct})

            logger.debug(f"Task {task_id}: Image {idx+1} compressed - {len(raw_bytes)} -> {len(compressed)} bytes")

        logger.info(f"Task {task_id}: Compressed {len(compressed_images)} images")
        return compressed_images

    except (ValidationError, CompressionError) as e:
        logger.error(f"Task {task_id}: Image compression failed: {e}")
        raise ImageProcessingError(f"Failed to compress images: {str(e)}")
    except Exception as e:
        logger.error(f"Task {task_id}: Unexpected compression error: {e}")
        raise
