...
from app.utils.ai_sanitizer import sanitize_user_input
...
# ─── SYNC HELPER FUNCTIONS FOR GEVENT COMPATIBILITY ─────────────────────────

def extract_food_info_using_ai_sync(text: str):
    """SYNC version of extract_food_info_using_ai for Celery workers with gevent"""
    import re
    ...
    try:
        ...
        logger.warning(f"OpenAI extraction error: {e}, using fallback parsing")
        return parse_food_with_smart_units(text)


def calculate_nutrition_using_ai_sync(food_name, quantity, unit):
    """SYNC version of calculate_nutrition_using_ai for Celery workers with gevent"""
    import re
    from app.fittbot_api.v1.client.client_api.chatbot.codes.food_log import (
...
def store_diet_data_to_db_sync(db, redis_client, client_id: int, date_str: str, logged_foods: list, meal: str):
    """SYNC version of store_diet_data_to_db for Celery workers"""
    from datetime import datetime
    from app.fittbot_api.v1.client.client_api.chatbot.codes.food_log import (
        get_default_diet_structure
    )
    from app.models.models import ActualDiet
    ...
    try:
        ...
        # Invalidate cache
        cache_key = f"actual_diet:{client_id}:{date_str}"
...
def process_voice_message(self, user_id: int, audio_bytes: bytes, meal: str = None):
    """
    Process voice message for food logging using SYNC clients (gevent-friendly).
    """
    ...
    try:
        ...
        # Step 2: Extract food info with SYNC OpenAI call
        logger.debug(f"Task {task_id}: Extracting food info from transcript using OpenAI")

        food_info = extract_food_info_using_ai_sync(transcript)
        ...
        # Step 3: Calculate nutrition for foods without it using SYNC call
        for food in foods:
            if food.get("quantity") is not None and food.get("calories") is None:
                nutrition = calculate_nutrition_using_ai_sync(
                    food["name"],
                    food["quantity"],
                    food["unit"]
                )
        ...
        try:
            ...
            if meal:
                store_diet_data_to_db_sync(db, redis_client, user_id, today_date, foods, meal)
        finally:
            db.close()
...
def extract_food_from_text(
    ...
    try:
        ...
        food_info = extract_food_info_using_ai_sync(text)
...
def transcribe_and_translate(self, user_id: int, audio_bytes: bytes, context: str = "general"):
    ...
    try:
        ...
        # SYNC OpenAI call - gevent yields during I/O
        try:
            response = sync_openai_call(
                oai_client,
                model="gpt-4o-mini",
...
@celery_app.task(bind=True, name="app.tasks.voice_tasks.process_food_template_voice")
