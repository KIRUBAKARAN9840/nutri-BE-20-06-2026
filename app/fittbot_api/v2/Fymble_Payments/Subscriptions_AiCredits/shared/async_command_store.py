"""
Async Redis-backed command store for FastAPI handlers.

Unlike the v1 CommandStore that wraps sync Redis with asyncio.to_thread,
this store uses the native async Redis client directly — zero thread hops.

Celery workers continue to use the sync CommandStore from v1.
"""
...
from ..credits.shared.schemas import (
    CommandStatus,
...
class AsyncCommandStore:
    """
    Native async Redis command store for FastAPI.
    Same data format as v1 CommandStore — Celery workers can read/write
    the same keys via the sync store.
    """
    ...
    def __init__(
        ...
        *,
        ...
        command_id_prefix: str = "cr_cmd",
    ):
        self.redis = redis
        self.config = config
        self._redis_prefix = redis_prefix or config.credits_redis_prefix
    ...
    async def get(
        self, command_id: str, *, owner_id: Optional[str] = None
    ) -> Optional[CommandRecord]:
        raw = await self.redis.get(self._key(command_id))
        if not raw:
            return None
        record = self._deserialize(json.loads(raw))
        if owner_id is not None:
            if not record.owner_id or record.owner_id != str(owner_id):
                return None
        return record
    ...
    async def mark_failed(
        self, command_id: str, error: str, *, result: Optional[Dict[str, Any]] = None
    ) -> CommandRecord:
        return await self._update(
            command_id,
    ...
    async def mark_pending_webhook(self, command_id: str) -> CommandRecord:
        """Mark command as waiting for webhook — unique to the delayed-task pattern."""
        return await self._update(
            command_id, status="pending_webhook"
        )
    ...
    async def _update(self, command_id: str, **updates) -> CommandRecord:
        ...
        data = {
            ...
            if isinstance(record.status, CommandStatus)
            else record.status,
    ...
    def _determine_owner_id(
        self, payload: Dict[str, Any], owner_id: Optional[str]
    ) -> Optional[str]:
