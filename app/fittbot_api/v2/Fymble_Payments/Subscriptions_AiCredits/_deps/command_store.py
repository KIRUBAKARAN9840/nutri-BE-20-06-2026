"""
Sync Redis-backed command store for Celery workers.
"""
...
from ..credits.shared.schemas import CommandStatus, CommandStatusResponse
...
class CommandStore:
        ...
        *,
        ...
        if idempotency_key:
            ...
            command_id = self._namespaced_id(
                generate_unique_id(self._command_id_prefix), owner
            )
        record = {
    ...
    async def mark_completed(
        ...
        return await self._update(
            command_id, status=CommandStatus.completed.value, result=result, error=None
        )
    ...
    async def mark_failed(
        ...
        return await self._update(
            command_id, status=CommandStatus.failed.value, error=error, result=result
        )
