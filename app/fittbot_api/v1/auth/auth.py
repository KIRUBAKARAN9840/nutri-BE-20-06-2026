...
from app.utils.otp_security import (
    secure_generate_otp, check_otp_send_allowed, secure_verify_otp,
)
...
async def set_trainer_password(request: SetTrainerPasswordRequest, db: Session = Depends(get_db), redis: Redis = Depends(get_redis)):
    try:
        ...
        mobile_otp = secure_generate_otp(mobile_number)
        await redis.set(f"otp:{mobile_number}", mobile_otp, ex=OTP_EXPIRY_SECONDS)
        ...
        if existing_client:
            ...
            if verification["mobile"]:
                ...
                if not verified:
                    ...
                    db.commit()
                    mobile_otp=secure_generate_otp(contact)
                    await redis.set(f"otp:{contact}", mobile_otp, ex=OTP_EXPIRY_SECONDS)
...
async def verify_otp(request: VerifyRequest, db: Session = Depends(get_db), redis=Depends(get_redis)):
    ...
    try:
        ...
        otp_result = await secure_verify_otp(redis, f"otp:{data}", otp, identifier=data)
        if otp_result.success:
            ...
            raise HTTPException(
                status_code=429 if otp_result.locked else 400,
                detail=otp_result.error_message
            )
...
async def send_otp( request:OTPRequest, db: Session = Depends(get_db), redis=Depends(get_redis)):
    ...
    try:
        send_check = await check_otp_send_allowed(redis, request.data)
        if not send_check.allowed:
            raise HTTPException(status_code=429, detail=send_check.reason, headers={"Retry-After": str(send_check.retry_after)})

        if await redis.exists(f"otp:{request.data}"):
            await redis.delete(f"otp:{request.data}")

        otp= secure_generate_otp(request.data)
        await redis.set(f"otp:{request.data}", otp, ex=OTP_EXPIRY_SECONDS)
...
async def send_otp( request:forgotRequest, db: Session = Depends(get_db), redis=Depends(get_redis)):
    ...
    try:
        ...
        send_check = await check_otp_send_allowed(redis, data)
        if not send_check.allowed:
            raise HTTPException(status_code=429, detail=send_check.reason, headers={"Retry-After": str(send_check.retry_after)})

        otp= secure_generate_otp(data)
        await redis.set(f"otp:{data}", otp, ex=OTP_EXPIRY_SECONDS)
...
async def verify_otp(request:VerifyRequest, redis=Depends(get_redis)):
    ...
    try:
        ...
        if otp_result.success:
            ...
            raise HTTPException(status_code=429 if otp_result.locked else 400, detail=otp_result.error_message)
...
async def send_verification_otp(request:EmailOtpData, db: Session = Depends(get_db), redis=Depends(get_redis)):
    try:
        ...
        if request.role == "client":
            ...
            otp=secure_generate_otp(request.email)
            await redis.set(f"otp:{request.email}", otp, ex=OTP_EXPIRY_SECONDS)
...
async def register_gym_owner(registration: GymOwnerRegistration, db: Session = Depends(get_db), redis=Depends(get_redis)):
    ...
    try:
        ...
        otp=secure_generate_otp(registration.mobile)
        await redis.set(f"otp:{registration.mobile}", otp, ex=OTP_EXPIRY_SECONDS)
