...
class SubscriptionSyncService:
    ...
    def process_initial_purchase(
        ...
        product_id, transaction_id, price, purchase_date, expires_date, is_trial = self.extract_purchase_details(event)
        ...
        try:
            ...
            if existing_subscription:
                self.logger.info("Customer %s already has subscription for %s", customer_id, product_id)
                ...
                if is_trial:
                    ...
                    existing_subscription.trial_start = purchase_date
                    existing_subscription.trial_end = expires_date
                else:
            ...
            if is_trial:
                # ── TRIAL PATH: subscription + entitlement only, no order/payment ──
                subscription = self.create_subscription(
                    customer_id, product_id, transaction_id,
                    purchase_date, expires_date, is_trial=True,
                )
                ...
                if processing_log:
                    ...
                    processing_log.result_summary = (
                        f"Trial: Subscription={subscription.id}, Entitlement={entitlement.id}"
                    )

                self.db.commit()
                self.logger.info("Successfully processed trial purchase for %s", customer_id)
                return {
                    "success": True,
            ...
            # ── PAID PATH: full order + payment + subscription + entitlement ──
            ...
            self.db.commit()
            self.logger.info("Successfully processed initial purchase for %s", customer_id)
            ...
            self.logger.error("Error processing initial purchase: %s", str(e))
            self.db.rollback()
    ...
    def extract_purchase_details(self, event: Dict[str, Any]) -> Tuple:
        ...
        Returns: (product_id, transaction_id, price, purchase_date, expires_date, is_trial)

        Dates MUST come from the webhook/provider — never generated randomly.
        """
        product_id = None
        transaction_id = None
        price = 0
        ...
        # Detect trial from period_type
        period_type = (event.get("period_type") or "").upper()
        if period_type == "TRIAL":
            is_trial = True

        # Timestamps — always from webhook event
        ...
        if not product_id or not transaction_id:
            ...
            for sub_key, sub_data in subscriptions.items():
                if sub_data:
                    ...
                    if not purchase_date and sub_data.get("purchased_at_ms"):
                        purchase_date = timestamp_to_ist(sub_data["purchased_at_ms"])
                    if not expires_date and sub_data.get("expiration_at_ms"):
                        expires_date = timestamp_to_ist(sub_data["expiration_at_ms"])
                    sub_period = (sub_data.get("period_type") or "").upper()
                    if sub_period == "TRIAL":
                        is_trial = True
                    break
        ...
        # Dates are required — if missing, log error and let caller handle
        if not purchase_date:
            self.logger.error("MISSING purchased_at_ms in event for customer %s", event.get("app_user_id"))
            purchase_date = now_ist()  # last-resort fallback, should not happen
        if not expires_date:
            self.logger.error("MISSING expiration_at_ms in event for customer %s", event.get("app_user_id"))
            expires_date = now_ist() + timedelta(days=30)  # last-resort fallback

        return product_id, transaction_id, price, purchase_date, expires_date, is_trial
    ...
    def create_subscription(
        ...
        """Create subscription. Sets status=trial and trial dates when is_trial=True."""
        ...
        subscription = Subscription(
            ...
            trial_start=purchase_date if is_trial else None,
            trial_end=expires_date if is_trial else None,
        )
        self.db.add(subscription)
        self.db.flush()
        self.logger.info("Created subscription %s (trial=%s)", subscription_id, is_trial)
        return subscription
    ...
    def process_renewal(
        ...
        try:
            ...
            if not purchased_at_ms or not expiration_at_ms:
                self.logger.error("RENEWAL_MISSING_DATES customer=%s purchased_at_ms=%s expiration_at_ms=%s",
                                  customer_id, purchased_at_ms, expiration_at_ms)
            ...
            self.logger.info("RENEWAL dates: purchased=%s expires=%s", purchased_date.isoformat(), expires_date.isoformat())
            
            # Find existing subscription (include trial status for trial-to-paid conversion)
            ...
            if subscription:
                ...
                was_trial = subscription.status == SubscriptionStatus.trial.value
                ...
                self.logger.info("Updated subscription %s (was_trial=%s):", subscription.id, was_trial)
                self.logger.info("   - active_until set to: %s", subscription.active_until.isoformat())
                ...
                # Ensure catalog product exists before creating order item
                self.create_or_update_catalog_product(product_id, amount_minor)
                ...
                if processing_log:
                    processing_log.status = "completed"
                    processing_log.completed_at = now_ist()
                    processing_log.result_summary = f"Renewed subscription {subscription.id} (trial_converted={was_trial})"

                self.db.commit()
                self.logger.info("Successfully processed renewal for %s (trial_converted=%s)", customer_id, was_trial)
    ...
    def process_cancellation(
        ...
        try:
            # Find subscription (include trial for trial cancellations)
    ...
    def process_expiration(
        ...
        try:
            ...
            # Find subscription (include trial for trial expirations)
            subscription = self.lock_for_update(
                self.db.query(Subscription).filter(
                    Subscription.customer_id == customer_id,
                    Subscription.product_id == product_id,
