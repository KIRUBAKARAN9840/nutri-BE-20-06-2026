...
async def enqueue_session_checkout(
    ...
    try:
        ...
        if body.session_type == SessionType.custom:
            ...
            if not body.schedule_id and not body.default_slot:
                log_request_error(
                    "CHECKOUT_ENQUEUE",
                    request_id,
                    SessionErrorCodes.EMPTY_SCHEDULED_SESSIONS,
                    "schedule_id or default_slot is required when session_type is 'same_time'",
                    start_time,
                )
                raise_structured_error(
                    status_code=400,
                    error_code=SessionErrorCodes.EMPTY_SCHEDULED_SESSIONS,
                    message="schedule_id or default_slot is required when session_type is 'same_time'",
                    field="schedule_id",
                    request_id=request_id,
                )
