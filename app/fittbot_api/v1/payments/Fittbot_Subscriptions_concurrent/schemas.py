...
class SessionCheckoutRequest(BaseModel):
    ...
    schedule_id: Optional[int] = None  # Direct schedule_id from v2 listing — skips time-string lookup
    default_slot: Optional[Any] = None  # int (schedule_id) from new app, or time string "07:00 AM" from old app
...
class NutritionPurchaseCheckoutRequest(BaseModel):
    client_id: int
    booking_date: str  # "2026-03-20"
    schedule_id: int



class NutritionPurchaseVerifyRequest(BaseModel):
    razorpay_payment_id: str
    razorpay_order_id: str
    razorpay_signature: str


class NutritionPurchaseCommandAccepted(BaseModel):
    request_id: str
    status: CommandStatus
    status_url: str
    retry_after_seconds: int = 2
