...
class WebhookProcessor:
    ...
    async def _persist(self, body: Dict) -> None:
        ...
        # For payment.captured events, trigger fulfillment (dailypass, session, gym membership, or nutrition purchase)
        if body.get("event") == "payment.captured":
            ...
            await self._try_nutrition_purchase_fulfillment(body)
    ...
    async def _try_nutrition_purchase_fulfillment(self, body: Dict) -> None:
        """If the captured payment belongs to a nutrition purchase order, fulfill it now."""
        try:
            pay_entity = body.get("payload", {}).get("payment", {}).get("entity", {})
            razorpay_order_id = pay_entity.get("order_id")
            payment_id = pay_entity.get("id")
            ...
            # Check if this is a nutrition purchase order by looking at notes
            notes = pay_entity.get("notes", {})
            flow = notes.get("flow", "")
            is_nutrition_purchase = flow == "nutrition_purchase"

            # If notes don't have flow, check DB
            if not is_nutrition_purchase:
                with self._session_scope() as session:
                    order = session.query(Order).filter(
                        Order.provider_order_id == razorpay_order_id
                    ).first()
                    if order and order.order_metadata:
                        order_flow = order.order_metadata.get("order_info", {}).get("flow", "")
                        if order_flow == "nutrition_purchase":
                            is_nutrition_purchase = True

            if not is_nutrition_purchase:
                return

            logger.info(
                "NUTRITION_PURCHASE_WEBHOOK_FULFILLMENT_TRIGGERED",
                extra={
                    "razorpay_order_id": razorpay_order_id,
                    "payment_id": f"****{payment_id[-4:]}" if len(payment_id) > 4 else payment_id,
                },
            ...
            from .nutrition_purchase_processor import NutritionPurchaseProcessor
            processor = NutritionPurchaseProcessor(
                config=self.config, payment_db=self.payment_db, redis=self.redis
            )
            await processor.fulfill_from_webhook(razorpay_order_id, payment_id, payment_data)
            ...
            logger.exception(
                "NUTRITION_PURCHASE_WEBHOOK_FULFILLMENT_ERROR",
                extra={"razorpay_order_id": body.get("payload", {}).get("payment", {}).get("entity", {}).get("order_id")},
            )
