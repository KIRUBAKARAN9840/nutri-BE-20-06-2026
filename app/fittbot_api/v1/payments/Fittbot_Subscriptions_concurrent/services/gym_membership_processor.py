...
from app.models.fittbot_models import FittbotGymMembership, ReferralFittbotCash, NoCostEmi, GymPlans, GymFees, Client
...
from app.config.pricing import (
    ...
    get_walkaway_redis_key, WALKAWAY_DISCOUNT_PERCENT,
)
...
async def _get_plan_details_async(db: AsyncSession, gym_id: int, plan_id: int, redis: Optional[Redis] = None) -> Tuple[int, int, bool, int]:
    ...
    # Resolve owner offer price (plan-specific > gym-wide > plan.amount)
    offers_by_gym = await fetch_active_membership_offers(db, [gym_id])
    gym_offers = offers_by_gym.get(gym_id, {})
    resolved_amount, _ = resolve_offer_base_amount(plan_id, base_amount, gym_offers)
    ...
    if is_promo_plan:
        ...
        raw_per_month = (resolved_amount * multiplier) / duration
        rounded_per_month = round_per_month_price(raw_per_month)
        amount_with_markup = rounded_per_month * duration
    else:
        amount_with_markup = smart_round_price(resolved_amount * multiplier)

  
    daily_discount = get_daily_offer_discount()
    if daily_discount > 0:
        amount_with_markup = max(amount_with_markup - daily_discount, 0)
...
MEMBERSHIP_BONUS_CREDITS = 5
MEMBERSHIP_CREDIT_EXPIRY_DAYS = 3


async def _grant_membership_bonus_credits(
    db: AsyncSession, client_id: int, source_order_id: str
) -> None:
    """Grant 5 free credits when a gym membership / PT is purchased.

    Uses the caller's db session directly.  Idempotent via
    dedup on source_order_id + txn_type.
    """
    import uuid
    ...
    try:
        ...
        dedup_key = f"membership_credits_{source_order_id}"
        ...
        existing = (
            await db.execute(
                select(CreditLedger).where(
                    ...
                    CreditLedger.txn_type == "membership_bonus",
                )
            )
        ).scalars().first()
        if existing:
        ...
        balance_row.balance += MEMBERSHIP_BONUS_CREDITS
        balance_row.total_bonus += MEMBERSHIP_BONUS_CREDITS
        ...
        ledger = CreditLedger(
            id=unique_id,
            client_id=client_id,
            txn_type="membership_bonus",
            credits=MEMBERSHIP_BONUS_CREDITS,
            balance_after=new_balance,
            source_order_id=dedup_key,
            description=f"Gym membership bonus ({MEMBERSHIP_BONUS_CREDITS} free credits)",
            expires_at=None,
            created_at=ist_now,
        )
        db.add(ledger)
        ...
        # Invalidate home cache so updated credits show immediately
        try:
        ...
        logger.info(
            "MEMBERSHIP_CREDITS_GRANTED | client_id=%s credits=%d balance=%d order=%s",
            client_id, MEMBERSHIP_BONUS_CREDITS, new_balance, source_order_id,
        )
    except Exception as e:
        await db.rollback()
        logger.warning("MEMBERSHIP_CREDITS_FAILED | client_id=%s order=%s err=%s", client_id, source_order_id, e)
...
class GymMembershipVerificationResponse:
    ...
    def dict(self) -> Dict[str, Any]:
        return {
            ...
            "bonus_credits": MEMBERSHIP_BONUS_CREDITS,
...
class GymMembershipProcessor:
    ...
    async def _invalidate_membership_bookings_cache(self, client_id) -> None:
        """Clear gym membership bookings cache after purchase."""
        if not self.redis:
            return
        key = f"bookings:gym_membership:active:{client_id}"
        try:
            await asyncio.to_thread(self.redis.delete, key)
        except Exception:
            logger.warning("MEMBERSHIP_BOOKINGS_CACHE_DELETE_FAILED", extra={"client_id": client_id})

    async def _invalidate_home_data_cache(self, client_id) -> None:
        """Clear cached home response so updated credits show immediately."""
        if not self.redis:
            return
        try:
            pattern = f"home:data:{client_id}:*"
            keys = await asyncio.to_thread(self.redis.keys, pattern)
            ustate_keys = await asyncio.to_thread(self.redis.keys, f"home:v2:ustate:{client_id}")
            all_keys = (keys or []) + (ustate_keys or [])
            if all_keys:
                await asyncio.to_thread(self.redis.delete, *all_keys)
        except Exception:
            logger.warning("HOME_DATA_CACHE_DELETE_FAILED", extra={"client_id": client_id})
    ...
    async def _checkout_async(self, db: AsyncSession, payload: UnifiedMembershipRequest) -> Dict[str, Any]:
        ...
        logger.info(f"[GM_CHECKOUT_START] client={user_id}, gym={gym_id}, plan={plan_id}")
        ...
        # ── Walkaway 5% discount ──
        walkaway_discount_minor = 0
        if self.redis:
            walkaway_key = get_walkaway_redis_key(int(user_id))
            walkaway_active = bool(await asyncio.to_thread(self.redis.exists, walkaway_key))
            if walkaway_active:
                amount_rupees = service_amount_minor // 100
                walkaway_cut = int(amount_rupees * WALKAWAY_DISCOUNT_PERCENT / 100)
                walkaway_discount_minor = walkaway_cut * 100
                service_amount_minor = max(service_amount_minor - walkaway_discount_minor, 0)
                logger.info(f"[GM_WALKAWAY] client={user_id}, discount={walkaway_cut}rs")

        # ── Telecaller coupon discount ──
        coupon_info = None
        coupon_discount_minor = 0
        if payload.coupon_code:
            coupon_info = await validate_coupon(
                db, payload.coupon_code, int(user_id),
            )
            if not coupon_info:
                raise HTTPException(400, "Invalid or expired coupon code")

            # Apply discount in rupees (same as booking preview) then convert back to minor
            amount_rupees = service_amount_minor // 100
            discounted_rupees = apply_coupon_discount(amount_rupees, coupon_info["discount_percent"])
            coupon_discount_minor = (amount_rupees - discounted_rupees) * 100
            service_amount_minor = discounted_rupees * 100
            logger.info(f"[GM_COUPON] code={payload.coupon_code}, discount={coupon_info['discount_percent']}%, saved={coupon_discount_minor/100}rs")

        # NOTE: service_amount_minor already includes platform markup + smart rounding from _get_plan_details_async
        gross_before_rewards = service_amount_minor
        logger.info(f"[GM_PRICING] service={service_amount_minor/100}rs, gross={gross_before_rewards/100}rs")
        ...
        order_metadata = {
            "order_info": {
                "order_type": f"{service_type}_with_bonus_credits",
                "customer_id": user_id,
                "created_at": datetime.now(UTC).isoformat(),
                "currency": "INR",
                "flow": f"{service_type}_with_bonus_credits",
            },
            "order_composition": {
                "includes_gym_service": True,
                "service_type": service_type,
                "service_label": service_label,
                "includes_bonus_credits": True,
                "bonus_credits": MEMBERSHIP_BONUS_CREDITS,
                "items_count": 1,
            },
            "payment_summary": {
                ...
                "step_3_walkaway_deduction": {
                    "walkaway_applied": walkaway_discount_minor > 0,
                    "walkaway_discount_percent": WALKAWAY_DISCOUNT_PERCENT if walkaway_discount_minor > 0 else 0,
                    "walkaway_discount_minor": walkaway_discount_minor,
                    "walkaway_discount_rupees": walkaway_discount_minor / 100,
                },
                "step_4_coupon_deduction": {
                    "coupon_applied": coupon_info is not None,
                    "coupon_code": payload.coupon_code if coupon_info else None,
                    "coupon_id": coupon_info["coupon_id"] if coupon_info else None,
                    "coupon_discount_percent": coupon_info["discount_percent"] if coupon_info else 0,
                    "coupon_discount_minor": coupon_discount_minor,
                    "coupon_discount_rupees": coupon_discount_minor / 100,
                },
                "step_5_final_amount": {
        ...
        try:
            rzp_order = await rzp_create_order(
                ...
                notes={
                    ...
                    "flow": f"{service_type}_with_bonus_credits",
                    "service_type": service_type,
                    "bonus_credits": str(MEMBERSHIP_BONUS_CREDITS),
        ...
        client_row = (await db.execute(select(Client.contact).where(Client.client_id == int(user_id)))).scalar()
        ...
        response = {
            ...
            "bonus_credits": MEMBERSHIP_BONUS_CREDITS,
            "display_title": f"{service_label} + {MEMBERSHIP_BONUS_CREDITS} Free AI Credits",
            "prefill": prefill,
        }
    ...
    async def _verify_async(
        ...
        if existing_payment:
            ...
            # Check if service was actually activated - async query
            items = (
                await db.execute(
                ...
                if it.item_type in (ItemType.gym_membership, ItemType.pt_session):
                    ...
                    if existing_ent:
                        ...
                        if entitlement_id:
                            ...
                            logger.info(f"[FITTBOT_PAYMENTS_RECOVERY_CREATED] source_type={source_type}, entitlement_id={entitlement_id}")

                        await db.commit()
                        logger.info(f"[GM_CREATED_MISSING_SERVICE] Created service: {service_details.get('entitlement_id')}")
                        service_ok = True

                        # Grant bonus credits (fire-and-forget, recovery)
                        asyncio.create_task(
                            _grant_membership_bonus_credits(db, int(order.customer_id), order.id)
                        )

            logger.info(f"[GM_EXISTING_PAYMENT_COMPLETE] service_ok={service_ok}")
        ...
        try:
            ...
            for ei in early_items:
                if ei.item_type in (ItemType.gym_membership, ItemType.pt_session):
                    ...
                    if existing_ent_early:
                        ...
                        await self._invalidate_membership_bookings_cache(order.customer_id)
            ...
            # Mark telecaller coupon as used
            coupon_meta = (order.order_metadata or {}).get("payment_summary", {}).get(
                "step_4_coupon_deduction", {}
            )
            if coupon_meta.get("coupon_applied") and coupon_meta.get("coupon_id"):
                await mark_coupon_used(db, coupon_meta["coupon_id"], int(order.customer_id))
            ...
            # and would trigger a lazy-load on the dead async session (MissingGreenlet).
            ...
            try:
                ...
                if "Duplicate entry" in str(e):
                    await db.rollback()
                    logger.info(f"[GM_DUPLICATE_DETECTED] Duplicate entry detected for order {_order_db_id}, returning already_processed")
                    return GymMembershipVerificationResponse(
                        verified=True,
                        captured=True,
            ...
            if service_ok:
                await _grant_membership_bonus_credits(db, int(order.customer_id), order.id)
                await self._invalidate_home_data_cache(order.customer_id)
            ...
            if "Duplicate entry" in str(e):
                logger.info(
                    "[GM_VERIFY_RACE_CONDITION_CAUGHT] Webhook already fulfilled this payment",
                    extra={"payment_id": _mask_sensitive(pid), "order_id": oid},
                )
                # Re-read state from a fresh session. The original `order` object's
                # attributes are expired after rollback, and touching them would
                # trigger a lazy-load on the dead async session → MissingGreenlet.
                async with (create_celery_async_sessionmaker()()) as fresh_db:
                    fresh_order = (
                    ...
                    existing_ent = (
                        await fresh_db.execute(
                            select(Entitlement).where(
                                Entitlement.order_item_id.in_(
                                    select(OrderItem.id).where(OrderItem.order_id == fresh_order.id)
                                )
                            )
                        )
                    ).scalars().first()
                    return GymMembershipVerificationResponse(
                        ...
                        payment_id=pid,
                        service_activated=True,
                        service_details={"entitlement_id": existing_ent.id, "status": "active"} if existing_ent else None,
                        total_amount=fresh_order.gross_amount_minor,
                        currency="INR",
                        message="Payment already processed",
                    ).dict()
            raise
    ...
    async def fulfill_from_webhook(
        ...
        gym_fees, FittbotGymMembership, reward deduction,
        nutrition eligibility, bonus credits, cache invalidation, and owner notification.
        """
        SessionLocal = create_celery_async_sessionmaker()
        async with SessionLocal() as db:
            try:
    ...
    async def _fulfill_from_webhook_async(
        ...
        # 6b) Mark telecaller coupon as used
        coupon_meta = (order.order_metadata or {}).get("payment_summary", {}).get(
            "step_4_coupon_deduction", {}
        )
        if coupon_meta.get("coupon_applied") and coupon_meta.get("coupon_id"):
