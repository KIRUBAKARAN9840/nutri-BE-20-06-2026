...
from app.config.pricing import get_markup_multiplier, compute_dailypass_price_paise
...
async def _get_price_for_gym_async(db: AsyncSession, gym_id: int, is_offer_eligible: bool = False) -> int:
    # Charged per-day price (paise): max(owner_base, ₹90) × markup. No intro offer.
    # is_offer_eligible kept for signature compatibility but no longer used.
    rec = (
        await db.execute(
    ...
    # Shared with the v2 display path (compute_dailypass_price_rupees) so the
    # charged price always equals the shown price.
    return compute_dailypass_price_paise(int(rec.discount_price))


async def _get_actual_price_for_gym_async(db: AsyncSession, gym_id: int, is_offer_eligible: bool = False) -> int:
    # Gym owner's configured base price (paise), no markup. No intro offer.
    # is_offer_eligible kept for signature compatibility but no longer used.
    rec = (
        await db.execute(
            select(DailyPassPricing).where(DailyPassPricing.gym_id == str(gym_id))
...
async def _check_dailypass_offer_eligibility(db: AsyncSession, client_id: int, gym_id: int) -> tuple:
    ...
    Intro/gym offer removed: no booking-count, gym-opt-in or cap queries are run.
    Always returns not-eligible so pricing is owner_base (floored ₹90) + commission.
    """
    return False, 0

    # ── Legacy intro-offer eligibility (disabled) ──
    try:
...
async def _try_notify_gym_mate_session_paid(
    db: AsyncSession,
    order: Optional[Order],
    daily_pass_id: Optional[str],
) -> None:
    """If this order was created with a gym_mate_session_id link, flip
    the matching GymMate session to 'paid'. Wrapped so any failure here
    does NOT break payment fulfillment — payment is the source of truth.
    Idempotent: SessionsAPI.mark_paid_via_webhook returns silently if the
    session is already paid.
    """
    if not order or not daily_pass_id:
        return

    metadata = order.order_metadata or {}
    ext = metadata.get("external_session") or {}
    if ext.get("type") != "gym_mate":
        return
    raw_id = ext.get("id")
    try:
    ...
    try:
        ...
        sessions_api = build_sessions_api(db)
        await sessions_api.mark_paid_via_webhook(session_id, str(daily_pass_id))
        logger.info(
            "GYMMATE_SESSION_PAID_NOTIFIED",
            extra={"gym_mate_session_id": session_id, "daily_pass_id": daily_pass_id},
        )
    except Exception as exc:
        logger.warning(
            "GYMMATE_SESSION_PAID_NOTIFY_FAILED",
            extra={
                "gym_mate_session_id": raw_id,
...
class DailyPassProcessor:
            ...
            try:
                ...
                for it in items:
                    ...
                    if it.item_type == ItemType.daily_pass:
                        ...
                        await _try_notify_gym_mate_session_paid(db, order, daily_pass_id)
    ...
    async def _checkout_async(self, db: AsyncSession, payload: UnifiedCheckoutRequest) -> Dict[str, Any]:
        ...
        external_session_meta = None
        if getattr(payload, "gym_mate_session_id", None):
            external_session_meta = {
                "type": "gym_mate",
                "id": int(payload.gym_mate_session_id),
            }
        ...
        order_metadata = {
            ...
            "external_session": external_session_meta,
            "order_composition": {
        ...
        try:
            rzp_order = await rzp_create_order(
                ...
                notes={
                    ...
                    **(
                        {"gym_mate_session_id": str(payload.gym_mate_session_id)}
                        if getattr(payload, "gym_mate_session_id", None)
                        else {}
                    ),
                },
