...
class UnifiedCheckoutRequest(BaseModel):
    gymId: int = Field(...)
    # clientId is no longer trusted from the client. The /checkout endpoint
    # overrides it from the JWT before passing to the dispatcher. Kept
    # optional so old clients that still send it don't fail validation.
    ...
    # External-system linkage. When set, the resulting DailyPass triggers
    # a callback into that system after fulfillment. Currently only the
    # GymMate sessions module uses this.
    gym_mate_session_id: Optional[int] = Field(None)
