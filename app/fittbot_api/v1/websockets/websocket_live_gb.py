...
session_hub: WebSocketHub | None = None
live_hub: WebSocketHub | None = None
chat_hub: WebSocketHub | None = None

# Backward-compatible alias
PatternHub = WebSocketHub
...
async def _ensure_hubs() -> tuple[WebSocketHub, WebSocketHub, WebSocketHub]:
    global redis_pool, session_hub, live_hub, chat_hub
    if redis_pool is None:
        redis_pool = await _create_redis()
    if session_hub is None:
        session_hub = WebSocketHub(redis_pool, prefix="live_sessions:")
        await session_hub.start()
    if live_hub is None:
        live_hub = WebSocketHub(redis_pool, prefix="live:")
        await live_hub.start()
    if chat_hub is None:
        chat_hub = WebSocketHub(redis_pool, prefix="chat:")
