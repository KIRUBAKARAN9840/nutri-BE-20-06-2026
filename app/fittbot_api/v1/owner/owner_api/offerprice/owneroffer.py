...
async def activate_new_offer(
    ...
    """
    Activate the special offer for first new users.
    ...
    try:
        ...
        price = f"₹{OFFER_PRICE_RUPEES}" if request.mode == "dailypass" else "₹99"
        return ActivateOfferResponse(
            status=200,
            message=f"{price} {request.mode} offer activated for first {GYM_OFFER_USER_CAP} new users!",
