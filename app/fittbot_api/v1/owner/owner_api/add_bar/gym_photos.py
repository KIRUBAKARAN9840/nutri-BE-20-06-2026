...
from app.config.constants import PRESIGN_EXPIRY_SECONDS, CACHE_TTL_LONG
from app.services.s3_service import AWS_REGION, BUCKET_NAME, get_upload_s3
...
async def get_gym_photos(
    ...
    try:
        ...
        # Cache the result
        await redis.set(redis_key, json.dumps(photo_data), ex=CACHE_TTL_LONG)  # Cache for 24 hours
...
async def get_registration_photos_presigned_urls(
    ...
    try:
        ...
        for media_item in body.media:
            ...
            await redis.set(redis_key, json.dumps(temp_photo_data), ex=CACHE_TTL_LONG)  # 24 hours
...
async def confirm_registration_photo_upload(
    ...
    try:
        ...
        await redis.set(redis_key, json.dumps(photo_data), ex=CACHE_TTL_LONG)  # 24 hours
