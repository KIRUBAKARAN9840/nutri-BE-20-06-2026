...
async def _get_dailypass_bookings(
    ...
    query = (
        select(
            ...
            DailyPass.booking_type,
            DailyPass.head_count,
        )
    ...
    for row in rows:
        ...
        is_multipass = row.booking_type == "multi"
        headcount = row.head_count or 1
        ...
        bookings.append({
            ...
            "price": price_val,
            "multipass": is_multipass,
            "headcount": headcount,
        })

    return {
        "expectedCount": total_headcount,
        "bookings": bookings
    }
...
async def _get_dailypass_counts_by_date(
    ...
    query = (
        select(
            ...
            func.sum(DailyPass.head_count).label("count")
        )
        .select_from(DailyPassDay)
        .join(DailyPass, DailyPass.id == DailyPassDay.pass_id)
        .where(
    ...
    result = await db.execute(query)
    return {row.scheduled_date: int(row.count or 0) for row in result.all()}
