...
from app.config.constants import CACHE_TTL_LONG
...
async def get_attendance_data(
    ...
    if not attendance_data:
        ...
        await redis.expire(attendance_key, CACHE_TTL_LONG)
    else:
...
async def get_count_data(
    ...
    if not members_data:
        ...
        await redis.expire(members_key, CACHE_TTL_LONG)
...
async def expiry_summary(
    ...
    await redis.set(about_key, json.dumps(about_list), ex=CACHE_TTL_LONG)  # Cache for 24 hours
    await redis.set(expired_key, json.dumps(expired_list), ex=CACHE_TTL_LONG)
...
async def get_birthday_clients(
    ...
    if bday_data:
        ...
        await redis.set(bday_key, json.dumps(bday_clients), ex=CACHE_TTL_LONG)
