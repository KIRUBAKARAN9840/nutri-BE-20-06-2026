...
async def collection_summary(
    ...
    try:
        now = datetime.now()
        # debug controlled by query parameter
        debug_info = {}
        ...
        membership_filters = [
            ...
            or_(
                FittbotGymMembership.old_client.is_(False),
                FittbotGymMembership.old_client.is_(None),
            ),
        ]
        ...
        # Debug: Capture scope info
        if debug:
            from datetime import timedelta as td
            scope_details = {"scope": scope, "gym_id": gym_id}
            if scope == "current_month":
                scope_details["month"] = now.month
                scope_details["year"] = now.year
            elif scope == "current_week":
                start_of_week = now.date() - td(days=now.weekday())
                end_of_week = start_of_week + td(days=6)
                scope_details["start_date"] = str(start_of_week)
                scope_details["end_date"] = str(end_of_week)
            elif scope == "custom_interval":
                scope_details["start_date"] = str(start_date)
                scope_details["end_date"] = str(end_date)
            elif scope == "specific_month_year":
                scope_details["month"] = month
                scope_details["year"] = year
            debug_info["scope_details"] = scope_details
        ...
        # Debug: Get individual membership payments with client names
        if debug:
            memberships = (
                db.query(
                    ...
                    FittbotGymMembership.joined_at,
                    FittbotGymMembership.status,
                    Client.name.label("client_name"),
                )
                .outerjoin(Client, FittbotGymMembership.client_id == Client.client_id)
                .filter(*membership_filters)
                .all()
            )

            # Build a lookup dict for manual client names
            manual_client_ids = [
                int(m.client_id.replace("manual_", ""))
                for m in memberships
                if m.client_id and str(m.client_id).startswith("manual_")
            ]
            manual_client_names = {}
            if manual_client_ids:
                manual_clients = db.query(ManualClient.id, ManualClient.name).filter(
                    ManualClient.id.in_(manual_client_ids)
                ).all()
                manual_client_names = {mc.id: mc.name for mc in manual_clients}
            ...
            for m in memberships:
                ...
                # Get client name - check if manual client
                client_name = m.client_name
                if not client_name and m.client_id and str(m.client_id).startswith("manual_"):
                    manual_id = int(m.client_id.replace("manual_", ""))
                    client_name = manual_client_names.get(manual_id, "Manual Client")
                ...
                membership_list.append({
                    ...
                    "status": m.status,
                    "is_manual": str(m.client_id).startswith("manual_") if m.client_id else False,
                })
            debug_info["memberships"] = {
                "count": len(membership_list),
                "manual_sum": manual_sum,
                "db_sum": float(other_revenue),
                "match": abs(manual_sum - float(other_revenue)) < 0.01,
                "records": membership_list,
            }

            for idx, m in enumerate(membership_list, 1):
                manual_tag = " [MANUAL]" if m.get("is_manual") else ""
                print(f"  {idx}. {m['client_name']:<20}{manual_tag} | Amount: {m['amount']:>10.2f} | Joined: {m['joined_at']} | Status: {m['status']}")
        ...
        # Debug: Get individual daily pass payments with client names
        if debug:
            dailypasses = (
                db.query(
                    GymBusinessPayment.id,
                    GymBusinessPayment.client_id,
                    GymBusinessPayment.amount,
                    GymBusinessPayment.date,
                    Client.name.label("client_name"),
                )
                .outerjoin(Client, GymBusinessPayment.client_id == Client.client_id)
                .filter(*dailypass_filters)
                .all()
            )
            dailypass_list = []
            dailypass_manual_sum = 0.0
            for dp in dailypasses:
                amt = float(dp.amount or 0)
                dailypass_manual_sum += amt
                dailypass_list.append({
                    "id": dp.id,
                    "client_id": dp.client_id,
                    "client_name": dp.client_name or "Unknown",
                    "amount": amt,
                    "date": str(dp.date) if dp.date else None,
                })
            debug_info["dailypasses"] = {
                "count": len(dailypass_list),
                "manual_sum": dailypass_manual_sum,
                "db_sum": float(dailypass_revenue),
                "match": abs(dailypass_manual_sum - float(dailypass_revenue)) < 0.01,
                "records": dailypass_list,
            }
            print(f"\n[DEBUG] DAILY PASSES ({len(dailypass_list)} entries):")
            print(f"{'─'*80}")
            for idx, dp in enumerate(dailypass_list, 1):
                print(f"  {idx}. {dp['client_name']:<20} | Amount: {dp['amount']:>10.2f} | Date: {dp['date']}")
            print(f"{'─'*80}")
            print(f"[DEBUG] Daily Pass Manual Sum: {dailypass_manual_sum}")
            print(f"[DEBUG] Daily Pass DB Sum: {float(dailypass_revenue)}")
