...
def grant_nutrition_eligibility_sync(
    ...
    try:
        ...
        db.rollback()
        # IntegrityError from unique constraint = already granted (race condition)
        if "uq_nutrition_eligibility_client_source_id" in str(exc):
            logger.info(
                "[NUTRITION_ELIGIBILITY_DEDUP] Already granted (race caught by DB)",
