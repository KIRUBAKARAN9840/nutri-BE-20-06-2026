...
async def trigger_food_template_voice(client_id: int, voice_type: str, db: Session):
    ...
    try:
        ...
        if voice_pref == "1":  # Voice enabled
            ...
            asyncio.create_task(send_voice_notification_async(client_id, f"food_template_{voice_type}"))
...
async def translate_text_celery(user_id: int, text: str) -> dict:
    """Direct async translation (no Celery)"""
    try:
        from app.services.ai_services import translate_text_async
        return await translate_text_async(user_id, text)
    except Exception as e:
...
async def ai_intent_classifier_celery(user_id: int, user_input: str, current_state: str) -> dict:
    """Direct async intent classification (no Celery)"""
    try:
        from app.services.ai_services import classify_meal_intent_async
        return await classify_meal_intent_async(user_id, user_input, current_state)
    except Exception as e:
        print(f"Error in ai_intent_classifier: {e}")
        return {
            "intent": "unclear",
            "confidence": 0.0,
            "extracted_data": {},
...
I'll create a personalized 7-day meal template for you. First, are you vegetarian or non-vegetarian or eggetarian or vegan or ketogic or paleo or jain?"""
       ...
       if text:
           text = sanitize_user_input(text, max_length=1000)
