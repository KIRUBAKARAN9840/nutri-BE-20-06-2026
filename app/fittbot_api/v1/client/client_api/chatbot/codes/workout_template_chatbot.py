...
from app.utils.ai_rate_limiter import require_ai_rate_limit
...
async def trigger_workout_template_voice(client_id: int, voice_type: str, db: Session):
    """Trigger voice notification via direct async call for workout template events"""
    try:
        ...
        if voice_pref == "1":  # Voice enabled
            import asyncio
            from app.services.ai_services import send_voice_notification_async
            asyncio.create_task(send_voice_notification_async(client_id, f"workout_template_{voice_type}"))
...
async def _generate_ai_day_names(user_request: str, days_count: int, oai, model: str = "gpt-3.5-turbo", user_id: int = None) -> List[str]:
    """Generate creative day names using direct async OpenAI call"""
    try:
        from app.services.ai_services import generate_day_names_async
        result = await generate_day_names_async(user_id or 0, user_request, days_count)
...
@router.post("/voice/transcribe", dependencies=[Depends(require_ai_rate_limit)])
async def voice_transcribe(
    ...
    """Transcribe audio to text and translate to English - uses direct async call"""
    from app.services.ai_services import transcribe_and_translate_async

    audio_bytes = await audio.read()

    try:
        result = await transcribe_and_translate_async(user_id, audio_bytes, "workout")
        return {
            "transcript": sanitize_user_input(result.get("transcript", ""), max_length=1000),
            "detected_language": result.get("lang", "unknown"),
            "english_text": sanitize_user_input(result.get("english", ""), max_length=1000)
        }
    except Exception as e:
        raise HTTPException(500, f"Transcription failed: {str(e)}")

@router.post("/voice/stream", dependencies=[Depends(require_ai_rate_limit)])
async def voice_stream_sse(
    ...
    """Transcribe audio and process it through the workout template stream - uses direct async call"""
    from app.services.ai_services import transcribe_and_translate_async
    ...
    try:
        result = await transcribe_and_translate_async(user_id, audio_bytes, "workout")
        transcript = sanitize_user_input(result.get("english", result.get("transcript", "")), max_length=1000)
    except Exception as e:
        raise HTTPException(500, f"Transcription failed: {str(e)}")
...
# ═══════════════════════════════════════════════════════════════
@router.get("/workout_stream", dependencies=[Depends(require_ai_rate_limit)])
async def ultra_flexible_workout_stream(
   ...
   user_input = sanitize_user_input(text.strip(), max_length=1000)
   ...
   # APPLY_EDIT STATE
    ...
    async def _apply_edit():
        try:
            try:
                from app.services.ai_services import detect_edit_intent_type_async
                intent_type = await detect_edit_intent_type_async(user_id, user_input)
            except Exception as e:
                print(f"Intent detection failed: {e}")
                intent_type = "EXERCISE_CHANGE"
