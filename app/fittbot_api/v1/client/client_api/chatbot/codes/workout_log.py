...
# Import async AI services (direct FastAPI, no Celery)
from app.services.ai_services import (
    extract_exercises_async,
    extract_exercises_with_details_async,
    parse_sets_reps_async,
    send_voice_notification_async,
)
...
async def extract_exercises_celery(user_id: int, text: str, timeout: float = 30.0) -> list:
    """Direct async OpenAI call for extract_exercises"""
    try:
        return await extract_exercises_async(user_id, text)
    except Exception as e:
        logger.error(f"extract_exercises error for user {user_id}: {e}")
        return []


async def extract_exercises_with_details_celery(user_id: int, text: str, timeout: float = 30.0) -> list:
    """Direct async OpenAI call for extract_exercises_with_details"""
    try:
        return await extract_exercises_with_details_async(user_id, text)
    except Exception as e:
        logger.error(f"extract_exercises_with_details error for user {user_id}: {e}")
        return []


async def parse_sets_reps_celery(user_id: int, text: str, exercise_name: str = "", timeout: float = 30.0) -> dict:
    """Direct async OpenAI call for parse_sets_reps"""
    try:
        return await parse_sets_reps_async(user_id, text, exercise_name)
    except Exception as e:
        logger.error(f"parse_sets_reps error for user {user_id}: {e}")
...
async def trigger_workout_log_success_voice(user_id: int, duration_minutes: int, total_calories: float, exercises_count: int, db: Session):
    """Trigger voice notification via async task for workout logging success"""
    try:
        ...
        if voice_pref == "1":  # Voice enabled
            import asyncio
            asyncio.create_task(send_voice_notification_async(
                user_id, "workout_log_success",
                {"duration_minutes": duration_minutes, "total_calories": round(total_calories), "exercises_count": exercises_count}
            ))
        else:
