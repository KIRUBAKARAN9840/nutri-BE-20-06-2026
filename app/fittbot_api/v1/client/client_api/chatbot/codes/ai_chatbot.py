...
from app.utils.ai_sanitizer import MAX_MESSAGES_PER_CHAT, MAX_MESSAGE_CONTENT_LENGTH
    ...
    if mode == "analysis" and not is_plan_request(tlower):
        if is_followup_question(text):
            ...
            if dataset:
                ...
                # Direct async OpenAI call (no Celery overhead)
                if len(msgs) > MAX_MESSAGES_PER_CHAT:
                    msgs = msgs[:1] + msgs[-(MAX_MESSAGES_PER_CHAT - 1):]
                for msg in msgs:
                    if isinstance(msg.get("content"), str) and len(msg["content"]) > MAX_MESSAGE_CONTENT_LENGTH:
                        msg["content"] = msg["content"][:MAX_MESSAGE_CONTENT_LENGTH]
...
async def voice_transcribe(
    ...
    """Transcribe audio with Groq and translate to English - direct async"""
    # Step 1: Transcribe with Groq (direct async)
    transcript = await transcribe_audio(audio, http=http)
    if not transcript:
        raise HTTPException(400, "empty transcript")

    # Step 2: Translate to English with async OpenAI
    oai_client = get_openai_client()
    try:
        resp = await async_openai_call(
            oai_client,
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content":
        ...
        tdata = orjson.loads(resp.choices[0].message.content)
        lang = (tdata.get("lang") or "unknown").strip()
        english = (tdata.get("english") or transcript).strip()
    except Exception:
        lang = "unknown"
        english = transcript

    return {"transcript": english, "lang": lang, "english": english}

@router.post("/voice/stream_test", dependencies=[Depends(require_ai_rate_limit)])
async def voice_stream_sse(
    ...
    """Voice chat stream - direct async transcription + OpenAI"""
    # Step 1: Transcribe with Groq (direct async)
    transcript = await transcribe_audio(audio, http=http)
    if not transcript:
        raise HTTPException(400, "empty transcript")
    ...
    await mem.add(user_id, "user", english)
    msgs, _ = await build_messages(user_id, english, use_context=True, oai=oai, mem=mem)
    ...
    async def token_iter():
        try:
            ...
            await mem.add(user_id, "assistant", content)
            yield "event: done\ndata: [DONE]\n\n"
        except Exception:
