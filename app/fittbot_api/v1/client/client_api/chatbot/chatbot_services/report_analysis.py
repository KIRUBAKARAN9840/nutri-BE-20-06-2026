...
async def extract_date_range_from_text_celery(user_id: int, text: str) -> Optional[tuple[date, date]]:
    ...
    try:
        text = sanitize_user_input(text, max_length=500)
        today = date.today()
...
async def generate_analysis_report_celery(
    ...
    """
    Generate fitness analysis report.
    Direct async OpenAI call (no Celery overhead).
    ...
    Args:
        ...
        user_request: Optional user request for dynamic formatting
    ...
    try:
        user_request = sanitize_user_input(user_request, max_length=500)

        # Dynamic format generation based on user request
        def generate_dynamic_style(user_req: str, ds: dict) -> str:
...
async def generate_followup_response_celery(
    ...
    """
    from app.services.ai_services import generate_followup_response_async

    try:
        return await generate_followup_response_async(
            user_id=user_id,
            user_text=user_text,
            summary=summary,
            dataset=dataset or {},
        ...
        print(f"Error in generate_followup_response: {e}")
...
async def voice_transcribe_analysis(
    ...
    transcript = sanitize_user_input(transcript, max_length=500)
