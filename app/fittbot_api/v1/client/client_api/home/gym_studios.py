...
from app.config.constants import OFFER_PRICE_RUPEES, OFFER_PRICE_PAISE, GYM_OFFER_USER_CAP
    ...
    try:
        ...
        for row in rows:
            ...
            if row.dailypass:
                ...
                if row.discount_price == OFFER_PRICE_PAISE:  # offer price in paise
...
async def _build_price_sort_map(
    ...
    if use_dailypass:
        ...
        for gid, raw in zip(ids_to_price, raw_results):
            ...
            dp_under_50 = dp_unique_map.get(gid, 0) < GYM_OFFER_USER_CAP
            ...
            if dp_offer_active:
                dp_display_prices[gid] = OFFER_PRICE_RUPEES  # Promo price
            else:
    ...
    if use_session and ids_to_price:
        ...
        for row in sess_result.all():
            ...
            sess_under_50 = session_unique_map.get(gid, 0) < GYM_OFFER_USER_CAP
...
async def _list_gyms_handler(
    ...
    try:
                ...
                if enabled_dp_ids:
                    ...
                    for gym_id in enabled_dp_ids:
                        if offer_map_temp.get(gym_id) and offer_map_temp.get(gym_id).dailypass:
                            ...
                            if current_count < GYM_OFFER_USER_CAP:
                                remaining = GYM_OFFER_USER_CAP - current_count
        ...
        for gym in gyms:
            ...
            dp_under_50 = dp_current_count < GYM_OFFER_USER_CAP

            if gym.dailypass and dailypass_offer_enabled and dp_under_50:
                dp_remaining = GYM_OFFER_USER_CAP - dp_current_count
            ...
            if gym.dailypass:
                try:
                    ...
                    if pricing_record:
                        ...
                        if pricing_record.discount_price == OFFER_PRICE_PAISE:
                            base_actual_price = OFFER_PRICE_RUPEES
                        ...
                        if dailypass_low or daily_pass_offer_active:
                            gym_data["daily_pass_discount_price"] = OFFER_PRICE_RUPEES  # Fixed promo price
                        else:
            ...
            session_under_50 = session_current_count < GYM_OFFER_USER_CAP
            ...
            if session_offer_enabled and session_settings and session_under_50:
                session_remaining = GYM_OFFER_USER_CAP - session_current_count
...
async def get_gym_details(request: Request, gym_id: int, db: AsyncSession = Depends(get_async_db), redis: Redis = Depends(get_redis)):
    try:
        ...
        session_under_50_detail = session_current_count_detail < GYM_OFFER_USER_CAP
        ...
        if session_offer_enabled and has_session and session_under_50_detail:
            session_remaining_detail = GYM_OFFER_USER_CAP - session_current_count_detail
        ...
        if gym.dailypass:
            ...
            if pricing_record:
                ...
                # Special case: If discount_price is exactly ₹49, no markup (matches processor logic)
                if pricing_record.discount_price == OFFER_PRICE_PAISE:
                    daily_pass_actual_price = OFFER_PRICE_RUPEES
                ...
                dp_under_50_detail = dp_current_count_detail < GYM_OFFER_USER_CAP

                if dailypass_offer_enabled and dp_under_50_detail:
                    dp_remaining_detail = GYM_OFFER_USER_CAP - dp_current_count_detail
                ...
                if dailypass_offer_active:
                    daily_pass_discount_price = OFFER_PRICE_RUPEES  # Fixed promo price
                else:
