...
from app.models.adminmodels import Admins, Employees, UserLoginLogs
...
async def login(request: LoginRequest, req: Request, db: Session = Depends(get_db)):
    try:
        ...
        admin_query = db.query(Admins).filter(Admins.contact_number == mobile_number)

        # When source is provided, narrow the query by role so the correct row is picked
        if source == "admin":
            admin_query = admin_query.filter(Admins.role.in_(["admin", "support"]))
        elif source == "nutritionist":
            admin_query = admin_query.filter(Admins.role == "nutritionist")
        ...
        access_token = create_access_token({"sub": str(admin.admin_id), "role": role, "user_type": "admin", "session_id": session_id})
        refresh_token = create_refresh_token({"sub": str(admin.admin_id), "user_type": "admin", "session_id": session_id})
        ...
        try:
            UserLoginLogs.__table__.create(bind=db.get_bind(), checkfirst=True)
            ip_address = req.client.host if req.client else None
            device_info = req.headers.get("user-agent")
            new_log = UserLoginLogs(
            ...
            logger.error(f"Error logging active session in POST /login: {log_err}")
    ...
    try:
        ...
        if not session_id:
            ...
            try:
                ...
                logger.error(f"Error creating fallback session in POST /refresh: {log_err}")

        access_token = create_access_token({"sub": str(request.id), "role": user.role, "user_type": user_type, "session_id": session_id})
        refresh_token = create_refresh_token({"sub": str(request.id), "user_type": user_type, "session_id": session_id})
...
async def logout(response: Response, request: Request, db: Session = Depends(get_db)):
    ...
    try:
            ...
            try:
                ...
                if payload:
                    ...
                    if session_id:
                        try:
                            UserLoginLogs.__table__.create(bind=db.get_bind(), checkfirst=True)
                            log_record = db.query(UserLoginLogs).filter(
                                UserLoginLogs.session_id == session_id,
                                UserLoginLogs.status == "ACTIVE"
                            ).first()
                            if log_record:
                                log_record.status = "LOGGED_OUT"
                                db.commit()
                        except Exception as log_err:
                            logger.error(f"Error updating session tracking in logout: {log_err}")
            except Exception as inner_err:
                logger.error(f"Error processing token in logout: {inner_err}")
        ...
        logger.error(f"Error in logout main handler: {e}")
        return {
            "status": 200,
            "message": "Logged out successfully"
        }
        ...
        try:
            ...
            if not session_id:
                ...
                try:
                    ...
                    logger.error(f"Error creating fallback session in POST /refresh-cookie: {log_err}")

            new_access_token = create_access_token({"sub": str(user_id), "role": user.role, "user_type": user_type, "session_id": session_id})
...
async def get_current_user_from_cookie(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Robust fallback for legacy/alternative login tokens missing 'user_type'
        if not user_type:
            user = db.query(Admins).filter(Admins.admin_id == int(user_id)).first()
            if user:
                user_type = "admin"
...
async def verify_totp_login(request_data: TOTPVerifyRequest, req: Request, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error logging active session in POST /totp/verify: {log_err}")
