...
from app.fittbot_admin_api.revenue_service import (
    ...
    get_daily_booking_counts,
    get_daily_active_user_counts,
    get_peak_metric_days,
)
    ...
    try:
        ...
        async def get_active_count(start_date, end_date):
            try:
                ...
                # NOTE: Must use or_(gym_id != 1, gym_id.is_(None)) to include organic
                # users who have NULL gym_id. In SQL, NULL != 1 evaluates to UNKNOWN
                # (not TRUE), so a plain != 1 silently drops all NULL-gym users.
                # This matches the same logic used in /api/admin/users-stats/data.
                conditions = [
...
async def get_users_trends(
    before_date: Optional[str] = Query(None, description="Fetch days before this date (YYYY-MM-DD)"),
    limit: int = Query(10, description="Number of days to fetch", ge=1, le=100),
    db: AsyncSession = Depends(get_async_db)
):
  
    try:
        # Parse before_date or default to current date
        if before_date:
            try:
                before_date_obj = datetime.strptime(before_date, "%Y-%m-%d").date()
                end_date = before_date_obj - timedelta(days=1)
            except ValueError:
        ...
        # Convert to datetimes for index-optimal range queries
        start_datetime = datetime.combine(start_date, datetime.min.time())
        end_datetime = datetime.combine(end_date, datetime.max.time())

        # ── Query 1: Daily new signups grouped by date (single aggregation, no N+1) ──
        daily_stmt = (
            select(
            ...
            .order_by(func.date(Client.created_at).desc())
        )
        daily_result = await db.execute(daily_stmt)
        daily_rows = daily_result.all()

        # Build date → count dict using a comprehension (O(n) on at most `limit` rows)
        db_counts = {
            (r[0].strftime("%Y-%m-%d") if hasattr(r[0], "strftime") else str(r[0])): r[1]
            for r in daily_rows
        }

        # ── Query 2: Combined — cumulative total up to end_date + earliest signup date ──
        # Single round-trip replaces what were previously 2 separate queries.
        combined_stmt = select(
            func.count(Client.client_id).label("cumulative_total"),
            func.min(Client.created_at).label("earliest_signup")
        ).where(
            Client.created_at <= end_datetime
        )
        combined_result = await db.execute(combined_stmt)
        combined_row = combined_result.one()
        total_up_to_end = combined_row.cumulative_total or 0
        min_date_val = combined_row.earliest_signup

        # ── Gap-fill: build complete calendar list in Python (max `limit` iterations) ──
        trends = []
        curr = end_date
        cumulative = total_up_to_end
        while curr >= start_date:
            date_str = curr.strftime("%Y-%m-%d")
            new_users = db_counts.get(date_str, 0)
            trends.append({
                "date": date_str,
                "new_users": new_users,
        ...
        # ── has_more: compare start_date against the earliest known signup ──
        if min_date_val:
            min_date = min_date_val.date() if isinstance(min_date_val, datetime) else min_date_val
...
async def get_bookings_trends(
    ...
    try:
        ...
        # ── Delegate to shared helper (single UNION ALL query, no N+1) ──
        db_counts = await get_daily_booking_counts(db, start_date, end_date)

        # ── has_more: check earliest DailyPass date ──
        min_stmt = select(func.min(DailyPass.created_at))
        min_result = await db.execute(min_stmt)
        min_date_val = min_result.scalar()
        ...
        # ── Gap-fill in Python (max `limit` iterations) ──
        trends = []
        curr = end_date
        while curr >= start_date:
            date_str = curr.strftime("%Y-%m-%d")
            trends.append({
                "date": date_str,
                "bookings": db_counts.get(date_str, 0),
            })
            curr -= timedelta(days=1)
...
async def get_total_bookings_metric(
    ...
    try:
        ...
        count = await get_total_bookings_count(db, start_date_obj, end_date_obj)
        return {
            "success": True,
...
@router.get("/active-users-trends")
async def get_active_users_trends(
    before_date: Optional[str] = Query(None, description="Fetch days before this date (YYYY-MM-DD)"),
    limit: int = Query(10, description="Number of days to fetch", ge=1, le=100),
    db: AsyncSession = Depends(get_async_db)
):
    """
    Get day-wise active user counts with cursor-based pagination.
    Delegates login logic to get_daily_active_user_counts() in revenue_service.py.
    """
    try:
        if before_date:
            try:
        ...
        # ── Delegate to shared helper (grouped by day, distinct client_ids) ──
        db_counts = await get_daily_active_user_counts(db, start_date, end_date)

        # ── has_more: check earliest login date in ActiveUser table ──
        min_stmt = select(func.min(ActiveUser.created_at))
        min_result = await db.execute(min_stmt)
        min_date_val = min_result.scalar()
        ...
        while curr >= start_date:
            ...
            trends.append({
                "date": date_str,
                "active_users": db_counts.get(date_str, 0),
            })
            curr -= timedelta(days=1)
...
async def get_peak_days_endpoint(db: AsyncSession = Depends(get_async_db)):
    """
    Get peak signup, active, and booking days.
    """
    try:
        peaks = await get_peak_metric_days(db)
        return {
            "success": True,
            "data": peaks
        }
...
async def get_holidays_endpoint(year: int):
    """
    Get public holidays and common regional/cultural Indian festivals for the specified year.
    Combines the Python 'holidays' library (with all Indian subdivisions and optional categories)
    and dynamic calendar rules without any hardcoded dates or years.
    """
    import holidays
    from datetime import date
    
    try:
        # Dictionary to accumulate holiday names by date object
        holiday_map = {}
        
        # 1. Fetch Indian public and optional holidays for the base country first
        try:
            ind = holidays.India(years=year, categories=['public', 'optional'])
            for d, name in ind.items():
                holiday_map.setdefault(d, set()).add(name)
        except Exception:
            pass
            
        # 2. Fetch holidays for all subdivisions (states/union territories) to cover regional festivals (like Pongal)
        try:
            for sub in holidays.India.subdivisions:
                try:
                    ind_sub = holidays.India(years=year, subdiv=sub, categories=['public', 'optional'])
                    for d, name in ind_sub.items():
                        holiday_map.setdefault(d, set()).add(name)
                except Exception:
                    pass
        except Exception:
        ...
        # 3. Add New Year's Day universally using a calendar formula for January 1st of the requested year
        try:
            holiday_map.setdefault(date(year, 1, 1), set()).add("New Year's Day")
        except Exception:
            pass

        # 4. Format list dynamically joining overlapping holidays
        formatted_holidays = []
        for d, name_set in sorted(holiday_map.items()):
            final_name = " / ".join(sorted(list(name_set)))
            formatted_holidays.append({
                "date": d.strftime("%Y-%m-%d"),
                "name": final_name,
                "localName": final_name
            })
    ...
    # Fallback to Nager.Date external API if library completely fails
...
async def get_user_by_id(user_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        stmt = select(
            ...
            latest_sub.c.product_id.label('subscription_product_id'),
            latest_sub.c.active_until.label('subscription_active_until'),
            GymMateProfile.bio.label('gymmate_bio')
        ).outerjoin(
            Gym, Client.gym_id == Gym.gym_id
        ).outerjoin(
            latest_sub, func.cast(Client.client_id, String) == latest_sub.c.customer_id
        ).outerjoin(
            GymMateProfile, Client.client_id == GymMateProfile.client_id
        ...
        # Fetch GymMate profile photos for this client
        photo_query = (
            select(
                GymMateProfilePhoto.id,
                GymMateProfilePhoto.s3_path,
                GymMateProfilePhoto.is_primary,
                GymMateProfilePhoto.display_order
            )
            .join(GymMateProfile, GymMateProfile.id == GymMateProfilePhoto.profile_id)
            .where(GymMateProfile.client_id == user_id)
            .order_by(GymMateProfilePhoto.display_order.asc())
        )
        photo_result = await db.execute(photo_query)
        photos_rows = photo_result.all()
        gymmate_photos = []
        base_s3_url = "https://fittbot-uploads.s3.ap-south-2.amazonaws.com/"
        for row in photos_rows:
            path = row.s3_path or ""
            if path and not path.startswith(base_s3_url):
                path = f"{base_s3_url}{path.lstrip('/')}"
            gymmate_photos.append({
                "id": row.id,
                "s3_path": path,
        ...
        user_data = {
            ...
            "gymmate_bio": result.gymmate_bio,
            "gymmate_photos": gymmate_photos
        }
...
@router.get("/{user_id}/hosted-sessions")
async def get_user_hosted_sessions(
    ...
    """Get gymmate sessions hosted by a specific user (host_client_id = user_id)
    along with session details and names of members (where role = 'member'),
    fully paginated and optimized.
    """
    try:
        from app.models.fittbot_models import GymMateSession, GymMateSessionMember, Client, Gym
        import math

        # 1. Query the total count of hosted sessions (efficient, single query)
        count_stmt = select(func.count(GymMateSession.id)).where(GymMateSession.host_client_id == user_id)
        count_result = await db.execute(count_stmt)
        ...
        if total_sessions == 0:
            return {
                ...
                "has_next": False,
                "has_prev": False,
                "message": "No hosted sessions found for this user"
            }

        # 2. Query only the sessions for the current page
        offset = (page - 1) * limit
        sessions_stmt = (
            select(GymMateSession, Gym.name.label("gym_name"))
            .outerjoin(Gym, Gym.gym_id == GymMateSession.gym_id)
            .where(GymMateSession.host_client_id == user_id)
            .order_by(desc(GymMateSession.session_date), desc(GymMateSession.session_time))
            .limit(limit)
            .offset(offset)
        )
        ...
        # 3. Extract session IDs of the paginated page only
        session_ids = [row.GymMateSession.id for row in sessions_rows]

        # Query all members for ONLY these paginated sessions where role == 'member'
        # (prevents N+1 database hits, uses single join)
        session_members_map = {}
        if session_ids:
            members_stmt = (
                select(GymMateSessionMember.session_id, GymMateSessionMember.client_id, Client.name.label("client_name"))
                .join(Client, Client.client_id == GymMateSessionMember.client_id)
                .where(
                    and_(
                        GymMateSessionMember.session_id.in_(session_ids),
                        GymMateSessionMember.role == "member"
                    )
                )
            )
            ...
            # Group members by session_id
            for row in members_rows:
                s_id = row.session_id
                if s_id not in session_members_map:
                    session_members_map[s_id] = []
                session_members_map[s_id].append({
                    "client_id": row.client_id,
                    "name": row.client_name
                })

        # 4. Format the final output for the current page
        formatted_sessions = []
        for row in sessions_rows:
            session = row.GymMateSession
            gym_name = row.gym_name
            s_id = session.id
            
            # Get members for this session
            members = session_members_map.get(s_id, [])
            ...
            formatted_sessions.append({
                ...
                "session_date": safe_isoformat(session.session_date),
                "session_time": safe_isoformat(session.session_time),
                "mate_preference": session.mate_preference,
                "fitness_level": session.fitness_level,
                "workout_vibes": session.workout_vibes,
        ...
        total_pages = math.ceil(total_sessions / limit) if limit > 0 else 0
        ...
        return {
            ...
            "has_next": page < total_pages,
            "has_prev": page > 1,
            "message": "Hosted sessions fetched successfully"
        }

    except Exception as e:
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error fetching hosted sessions: {str(e)}")
